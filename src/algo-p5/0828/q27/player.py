...
class Player:
    ...
    def go_forward(self, cells):
        ...
        if event_nm == "BattleVsZako":
            ...
            # TODO:event_nmが"HealingLake"の場合、新たに定義したself.healed_in_lake()を呼び出してください。

        elif event_nm == "PoisonSwamp":
            # TODO:event_nmが"PoisonSwamp"の場合、新たに定義したself.poisoned_in_swamp()を呼び出してください。
    ...
    # TODO:healed_in_lakeメソッドを定義します。引数はselfのみです。
    def healed_in_lake(self):
        ...
        なし
        """
        # TODO:「イベント発生！癒しの湖で身を清めます。」を表示してください。


        # TODO:HPを最大まで回復します。self.hpにself.max_hpを代入してください。


        # TODO:「(self.name)のHPが全回復した!現在のHPは(self.hp)です。」を表示してください。


    # TODO:poisoned_in_swampメソッドを定義します。引数はselfのみです。
    def poisoned_in_swamp(self):
        ...
        なし
        """
        # TODO:「イベント発生！沼で毒に冒されました。」を表示してください。


        # TODO:20のダメージを受けます。self.hpから20を引いて(self.hpに再代入して)ください。


        if self.hp > 0:
            # TODO:self.hpが0より大きい場合、「(self.name)は20のダメージを受けた!現在のHPは(self.hp)です。」を表示してください。

        else:
            # TODO:上記以外の場合、「(self.name)は20のダメージを受けた!(self.name)はしんでしまった!世界は闇に包まれてしまった...」を表示してください。

            # TODO:ゲームオーバーなので終了です。1つ前のメッセージに続けて、sys.exit()を呼び出してください。
