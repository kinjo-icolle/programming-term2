...
class Player:
    ...
    # TODO:attackメソッドを定義します。引数はself, enemyです。
    def attack(self, enemy):
        ...
        """
        # TODO:相手に与えるダメージとして、変数damageに、self.min_damageを代入する
        damage = self.min_damage

        # TODO:「(self.name)のこうげき!」を表示
        print(self.name + "のこうげき!")

        # TODO:enemy.hpからdamageを引いてください。(引いた結果をenemy.hpに再代入してください)
        enemy.hp -= damage

        if enemy.hp > 0:
            # TODO: enemy.hp > 0の場合、「(enemy.name)に(damage)のダメージを与えた!(enemy.name)のHPは(enemy.hp)です。」を表示した後、Falseを返す
            print(enemy.name + "に" + str(damage) + "のダメージを与えた!" + enemy.name + "のHPは" + str(enemy.hp) + "です。")
            return False
        else:
            # TODO: 上記以外の場合、「(enemy.name)に(damage)のダメージを与えた!(enemy.name)のHPは0です。」を表示した後、Trueを返す
...
if __name__ == '__main__':
    ...
    # TODO:kevinのattack()メソッドを、引数にenemyを与えることで、「スラスラ」を攻撃してください。
    kevin.attack(enemy)

    # TODO:enemyのattack()メソッドを、引数にkevinを与えることで、「ケビン」を攻撃してください。
    enemy.attack(kevin)
