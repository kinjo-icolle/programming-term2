...
class Player:
    ...
    def battle(self, enemy):
        ...
        # 敵が倒されるまで戦い続ける
        while enemy.hp > 0:
            # TODO:見やすさのために、print()を引数なしで呼び出し、空行を表示してください
            print()

            # TODO:「どうする?」を表示してください
            print("どうする?")

            # TODO:input()関数を用い、「1:攻撃する、9:逃げる>> 」を表示し、入力された値を変数cmd_numに代入してください。
            cmd_num = input("1:攻撃する、9:逃げる>> ")

            # TODO:cmd_numの値によって条件判断してください。
            if cmd_num == "1":
                # TODO:cmd_numが1の場合、これまで通り敵を攻撃してください。
                # プレイヤーが敵を攻撃。倒したらループを抜ける
                if self.attack(enemy):
                    break
            elif cmd_num == "9":
                # TODO:cmd_numが9の場合、「(self.name)は逃げ出した!」を表示し、return(のみ記述し、値は何も返さない)してください。
